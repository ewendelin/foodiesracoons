...
json.claim do
  json.claimed_at @claim.claimed_at.in_time_zone("Beijing").strftime('%e %b %Y %H:%M%p')
end
...
json.post do
  ...
  json.start_time @post.start_time.in_time_zone("Beijing").strftime('%H:%M%p')
  json.end_time @post.end_time.in_time_zone("Beijing").strftime('%H:%M%p')
end
